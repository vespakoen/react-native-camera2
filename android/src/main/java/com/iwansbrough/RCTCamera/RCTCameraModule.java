...
public class RCTCameraModule extends ReactContextBaseJavaModule implements MediaRecorder.OnInfoListener {
    ...
    private void releaseMediaRecorder() {
        ...
        //                 mRecordingPromise.resolve(mReactAppContext.getContentResolver().insert(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, values).toString());
        //                 mReactAppContext.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://"+ mVideoFile.getPath())));
        //                 break;
        //             case RCT_CAMERA_CAPTURE_TARGET_TEMP:
        //             case RCT_CAMERA_CAPTURE_TARGET_DISK:
        //             default:
    ...
    public void capture(final ReadableMap options, final Promise promise) {
        ...
        //                         mReactAppContext.getContentResolver(),
    ...
    private File getTempMediaFile(int type) {
        try {
            ...
            if (type == MEDIA_TYPE_IMAGE) {
                outputDir = mReactAppContext.getCacheDir(); // for backwards compatibility
                outputFile = File.createTempFile("IMG_" + timeStamp, ".jpg", outputDir);
            } else if (type == MEDIA_TYPE_VIDEO) {
                outputDir = mReactAppContext.getFilesDir(); // for compatibility with react-native-fs
